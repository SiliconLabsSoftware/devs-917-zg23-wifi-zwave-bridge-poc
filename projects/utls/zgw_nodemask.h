...
#define is_classic_node(nodeid) \
  (((nodeid) > 0) && ((nodeid) <= ZW_CLASSIC_MAX_NODES))
 ...
 */
#define nodemask_nodeid_is_valid(nodeid) \
  (is_classic_node(nodeid) || is_lr_node(nodeid))

/**
 * Test if a nodeid is outside the valid range (1 to ZW_MAX_NODES).
 */
#define nodemask_nodeid_is_invalid(nodeid) \
  ((!is_classic_node(nodeid)) && (!is_lr_node(nodeid)))
 ...
 */
typedef uint8_t  nodemask_t[ZW_MAX_NODES / 8 + 1]; // +1 to accommodate LR node ID 4000
