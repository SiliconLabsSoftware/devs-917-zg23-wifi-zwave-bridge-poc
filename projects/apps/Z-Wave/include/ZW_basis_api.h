...
/*This bit mask used as parameters for the API ZW_SetSleepMode to indicate which interrupt to mask out*/
...
/*The valid values of the ApplicationRfNotify API*/
...
/* This are the first (protocol part) payload bytes from the Node Information frame */
...
**
**--------------------------------------------------------------------------*/
#define ZW_MAX_LR_TX_POWERLEVEL_SET(TXPOWERLEVEL) ZW_MAXLRTXPowerLevelSet(TXPOWERLEVEL)

/*==========================   ZW_MAXLRTXPowerLevelGet  ==========================
**    Get the current MAX LR TX powerlevel
**
*--------------------------------------------------------------------------*/
#define ZW_MAX_LR_TX_POWERLEVEL_GET() ZW_MAXLRTXPowerLevelGet()
 ...
 * is to call \ref sl_zw_get_node_proto_info to verify that Generic and specific nodetype
...
ZW_Version(
  BYTE *dst);   /* IN Destination pointer in RAM */
...
/*
 * \ingroup BASIS
 * \macro{ZW_MAX_LR_TX_POWERLEVEL_SET()}
 *
 * Configure the current MAX LR TX powerlevel
 *
 * \note This API is available as of serial API version 7
 *
 * \return 0 - failed, 1 - success
...
/*
 * \ingroup BASIS
 * \macro{ZW_MAX_LR_TX_POWERLEVEL_GET()}
 *
 * Get the current MAX LR transmit(TX) power level
 *
 * \note This API is available as of serial API version 7
 *
 * \return The Max LR TX powerlevel currently in effect
 *
 * \serialapi{
