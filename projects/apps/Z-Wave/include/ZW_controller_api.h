...
/*Set this flag in bMode to include a node in the Long range mode. */
#define ADD_NODE_OPTION_LR                   0x20

/* Set This flag indicate if the operation must skip FL nodes in neighbors discovery
 * of the included node*/
#define ADD_NODE_OPTION_SFLND                0x10

/* Callback statuses from ZW_AddNodeToNetwork */
...
#define ADD_NODE_STATUS_SFLND_DONE           0x0B
...
/* Node parameter to ZW_RemoveNodeIDFromNetwork */
...
**--------------------------------------------------------------------------*/
#define ZW_GET_NODE_STATE(nodeID, nodeInfo) sl_zw_get_node_proto_info(nodeID, nodeInfo)
...
**                                FALSE if a return route assign/delete is already active
** ZW_AssignReturnRoute(
...
**    TX_STATUS_TYPE *txStatusReport)); IN  Status of process
**--------------------------------------------------------------------------*/
#define ZW_ASSIGN_SUC_RETURN_ROUTE(routingNodeID, func) sl_zw_assign_SUC_route(routingNodeID, func)
...
**
**--------------------------------------------------------------------------*/
#define ZW_SEND_SUC_ID(node, txOption, callback) sl_zw_send_SUCID(node, txOption, callback)
...
**     If a node sends its node info, sl_appl_controller_update will be called
...
**      The returned capability is a bitmask where following bits are defined :
...
**    ADD_NODE_END_NODE          Add a slaev node to the network
...
**    REMOVE_NODE_END_NODE          Remove a slaev node from the network
**
...
**    REMOVE_NODE_END_NODE          Remove Specified nodeID (slave) from the network
**
**    REMOVE_NODE_STOP           Stop learn mode without reporting an error.
**
 ...
 * \ref sl_appl_controller_update to update local data structures.
 *
 * The Z Wave protocol MUST notify a controller application by calling
 * \ref sl_appl_controller_update when a new node has been added or deleted
 * from the controller through the network management features.
 *
 * The Z Wave protocol MUST call \ref sl_appl_controller_update in response
 ...
 * \ref sl_appl_controller_update when a Node Information Frame has been received.
 * The Z Wave protocol MAY refrain from calling the function if the protocol
 * is currently expecting a Node Information frame.
 *
 * \ref sl_appl_controller_update MUST be called in a controller node operating
 ...
 * to \ref sl_appl_controller_update in response to \ref ZW_RequestNetWorkUpdate.
 *
 * The Z Wave protocol MUST NOT call \ref sl_appl_controller_update in a
 ...
 *
 * sl_appl_controller_update via the Serial API also have the possibility for
...
ZW_ReplicationSend(
  uint16_t destNodeID,             /*IN  Destination node ID. Only single cast allowed*/
...
ZW_DeleteReturnRoute(
  uint16_t nodeID,        /*IN Routing Slave */
...
ZW_isFailedNode(
  uint16_t nodeID);   /* IN the failed node ID */
...
ZW_ReplaceFailedNode(
  uint16_t bNodeID,                            /* IN the nodeID on the failed node to replace */
...
ZW_SetSUCNodeID(
  ...
  BYTE bTxOption,    /* IN TRUE if to use low power transmission, FALSE for normal Tx power */
...
**
**--------------------------------------------------------------------------*/
uint16_t /*RET nodeID on SUC, if ZERO -> no SUC */
ZW_GetSUCNodeID(void);   /* IN Nothing */

/*========================   ZW_RequestNetWorkUpdate   ======================*/
/**
 * \ingroup BASIS
 * \def ZW_REQUEST_NETWORK_UPDATE (func)
 *
 ...
 * calling the sl_appl_controller_update).
 *
...
ZW_RequestNetWorkUpdate(
  VOID_CALLBACKFUNC(completedFunc)(BYTE txStatus)); /* call back function indicates of the update succeeded or failed*/
...
extern BOOL             /*RET TRUE if delete SUC return routes was initiated. */
                        /*    FALSE if a return route assign/delete is already active */
ZW_DeleteSUCReturnRoute(
  uint16_t bNodeID,        /*IN Routing Slave Node ID */
  VOID_CALLBACKFUNC(completedFunc)(     /*IN Callback function */
