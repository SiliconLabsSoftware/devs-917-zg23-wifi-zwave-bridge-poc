...
ZW_SetSlaveLearnMode(
  uint16_t node,          /* IN nodeID on Virtual node to set in Learn Node Mode - if new node wanted then it must be ZERO */
  BYTE mode,          /* IN TRUE  Enable, FALSE  Disable */
  VOID_CALLBACKFUNC(learnFunc)(BYTE bStatus, uint16_t orgID, uint16_t newID));  /* IN Slave node learn call back function. */
