...
typedef enum {
   ...
   * re-initialization of the gateway (sl_application_nif_init()), unlock RD
   ...
   * sl_nm_virtual_nodes_removed() will bring NMS back to
   * \ref NM_IDLE. */
 ...
 */
void sl_nm_net_update_status(uint8_t flag);
 ...
 *
 * Along with the boot process, sl_nm_init will be called several times. Each time it will
 ...
 */
void sl_nm_inif_received(nodeid_t bNodeID, uint8_t INIF_rxStatus,
