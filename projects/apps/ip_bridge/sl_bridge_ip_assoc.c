...
#include "apps/Net/ZW_udp_server.h"
...
static void sli_handle_ip_association_set(zwave_connection_t *c,
                                          const u8_t *payload,
                                          const u8_t len,
                                          BOOL was_dtls);
static void sli_handle_ip_association_get(zwave_connection_t *c,
                                          const u8_t *payload,
                                          const u8_t len);
static void sli_handle_ip_association_remove(zwave_connection_t *c,
                                             const u8_t *payload,
                                             const u8_t len);
static bridge_return_status_t
sli_ip_assoc_create(ip_assoc_type_t type,
                    ZW_COMMAND_IP_ASSOCIATION_SET *ip_assoc_set_cmd,
                    ip_assoc_cb_func_t create_completed_cb,
                    uint8_t was_dtls);
static ip_association_t *
sli_ip_assoc_create_local(ZW_COMMAND_IP_ASSOCIATION_SET *cmd);
...
static ip_association_t *ip_assoc_find_existing(ip_assoc_type_t type,
                                                uint8_t was_dtls);
static void ip_assoc_setup_and_save(
  ip_association_t *new_assoc,
...
static void send_ip_association_cmd_callback(unsigned char bStatus,
                                             ...
                                             TX_STATUS_TYPE *t);
static uint8_t
ip_assoc_setup_multi_channel_assoc_set(uint8_t *p,
                                       nodeid_t assoc_dest_node,
                                       ZW_COMMAND_IP_ASSOCIATION_SET *cmd);
static int ip_assoc_setup_classic_assoc_set_cmd(
  uint8_t *p,
...
static uint8_t
ip_assoc_setup_multi_channel_assoc_remove_cmd(uint8_t *p,
                                              ip_association_t *ipa);
...
void ip_assoc_unpersist_association_table(void)
{
  ...
  rd_datastore_unpersist_association(ip_association_table,
                                     &ip_association_pool);
}
                           ...
                           BOOL was_dtls)
{
  switch (payload[1]) {
    case ASSOCIATION_SET:
      sli_handle_ip_association_set(c, payload, len, was_dtls);
      return TRUE;
      break;

    case ASSOCIATION_REMOVE:
      sli_handle_ip_association_remove(c, payload, len);
      return TRUE;
      break;

    case ASSOCIATION_GET:
      sli_handle_ip_association_get(c, payload, len);
      return TRUE;
      break;
...
void ip_assoc_remove_by_nodeid(nodeid_t node_id)
{
  ...
  if (ip_assoc_is_any_marked_for_removal()) {
    ...
    zw_zip_post_event(ZIP_EVENT_NM_VIRT_NODE_REMOVE_DONE, 0);
  }
}
...
void ip_assoc_print_association_table(void)
{
  ...
  uint8_t line_no = 1;
  for (ip_association_t *ia = list_head(ip_association_table); ia != NULL;
       ia                   = list_item_next(ia), line_no++) {
 ...
 * ****** STATIC FUNCTIONS ******
 */
static int sli_hndle_ip_asso_multi(u8_t assoc_dest_ep,
                                   ...
                                   nodeid_t dest_nodeid)
{
   ...
   * AND = Association Destination Node
   *
   * There are 5 sub-cases here:
   *  Case 1: Single ASN, Single AND -> Classic Assoc Set
   *  Case 2: Single ASN no multi support, Multi AND -> Classic Assoc Set
   *          (ZIP Roter must proxy and provide Multichannel encap)
   *  Case 3: Multi ASN, Single AND -> Multi-channel encapsulated Classic Assoc Set
   *  Case 4: Multi ASN, Multi AND -> Multi-channel encapsulated Multi Channel Assoc Set
  ...
  if (c->lendpoint == 0 && assoc_dest_ep == 0) {
    /* if the ASN endpoint and AND endpoint are both zero (case 1).
    ...
    /* if the ASN endpoint is zero and AND endpoint is NOT zero. (case 5)
     ...
     */
    if (!(rd_ep_class_support(ep, COMMAND_CLASS_MULTI_CHANNEL_ASSOCIATION_V2)
          & SUPPORTED)) {
      ASSERT(0);
      return 0;
    }
  } else if (c->lendpoint != 0 && assoc_dest_ep == 0) {
    /* if the ASN endpoint is NOT zero and AND endpoint is zero. (case 3)
     ...
     */
    if ((!sl_cmdclass_supported(dest_nodeid, COMMAND_CLASS_MULTI_CHANNEL_V2)
         && !sl_cmdclass_secure_supported(dest_nodeid,
    ...
    /* if the ASN endpoint is NOT zero and AND endpoint NOT is zero. (case 4)
    ...
    if ((!sl_cmdclass_supported(dest_nodeid, COMMAND_CLASS_MULTI_CHANNEL_V2)
         && !sl_cmdclass_secure_supported(dest_nodeid,
                                          COMMAND_CLASS_MULTI_CHANNEL_V2))
        || !(rd_ep_class_support(ep, COMMAND_CLASS_MULTI_CHANNEL_ASSOCIATION_V2)
             & SUPPORTED)) {
      ASSERT(0);
      return 0;
    }
...
static void sli_handle_ip_asso_create(zwave_connection_t *c,
                                      u8_t assoc_dest_ep,
                                      u8_t *payload,
                                      BOOL was_dtls,
                                      ...
                                      nodeid_t dest_nodeid)
{
  int i = 0;
  ZW_COMMAND_IP_ASSOCIATION_SET *ip_assoc_set_cmd =
    (ZW_COMMAND_IP_ASSOCIATION_SET *) payload;
  if ((c->lendpoint == 0 && assoc_dest_ep != 0)
      && !sl_cmdclass_supported(dest_nodeid,
                                COMMAND_CLASS_MULTI_CHANNEL_ASSOCIATION_V2)
      && !sl_cmdclass_secure_supported(dest_nodeid,
                                       COMMAND_CLASS_MULTI_CHANNEL_ASSOCIATION_V2)) {
    /* Case 2 */
    /* we must proxy this connection */
    if (sli_ip_assoc_create(PROXY_IP_ASSOC,
                            ip_assoc_set_cmd,
  ...
  ip_association_t *a = sli_ip_assoc_create_local(ip_assoc_set_cmd);
...
static int sli_handle_ip_asso_rd_class(zwave_connection_t *c,
                                       const u8_t *payload,
                                       const u8_t len,
                                       BOOL was_dtls,
                                       rd_ep_database_entry_t *ep)
{
  (void) len;
  uint8_t pdata[len];
  memcpy(pdata, payload, len);
  ZW_COMMAND_IP_ASSOCIATION_SET *ip_assoc_set_cmd =
    (ZW_COMMAND_IP_ASSOCIATION_SET *) pdata;
  /* creating an IP Association that goes out of the PAN to a ZIP client;
   * and NOT to another node in the PAN. These are called Permanent associations
   */
  ...
  if (c->lendpoint == 0) {
    ...
    if (!(rd_ep_class_support(ep, COMMAND_CLASS_MULTI_CHANNEL_V2)
          & SUPPORTED)
        && !(rd_ep_class_support(ep, COMMAND_CLASS_ASSOCIATION_V2) & SUPPORTED)) {
      ASSERT(0);
      return 0;
   ...
   */
  if (sli_ip_assoc_create(PERMANENT_IP_ASSOC,
                          ip_assoc_set_cmd,
...
{
  (void) len;
  uip_ip6addr_t *assoc_dest_ip, ip;
  u8_t assoc_dest_ep;
  nodeid_t dest_nodeid;
  rd_ep_database_entry_t *ep;
  ...
  const ZW_COMMAND_IP_ASSOCIATION_SET *ip_assoc_set_cmd =
    (const ZW_COMMAND_IP_ASSOCIATION_SET *) pData;
  memcpy(&ip, &ip_assoc_set_cmd->resourceIP, sizeof(ip));
  assoc_dest_ip = &ip;
  assoc_dest_ep = ip_assoc_set_cmd->endpoint;
  dest_nodeid   = sl_node_of_ip(&c->lipaddr);
  ...
  if (is_local_address(assoc_dest_ip)) {
    if (sli_hndle_ip_asso_multi(assoc_dest_ep, c, ep, dest_nodeid) == 0) {
      goto abort;
    }

    return sli_handle_ip_asso_create(c,
                                     assoc_dest_ep,
                                     pData,
                                     was_dtls,
                                     assoc_dest_ip,
    ...
    if (sli_handle_ip_asso_rd_class(c, payload, len, was_dtls, ep) == 0) {
      goto abort;
    }
  }
  return;
                                          ...
                                          const u8_t len)
{
  ip_association_t *a;
  uint8_t zipdata[len];
  memcpy(zipdata, payload, len);
  ZW_IP_ASSOCIATION_GET_FRAME *req = (ZW_IP_ASSOCIATION_GET_FRAME *) zipdata;
  ...
  nodes_in_grouping = ip_assoc_find_by_grouping(sl_node_of_ip(&c->lipaddr),
                                                c->lendpoint,
                                                req->groupingIdentifier,
                                                req->index,
                                                &a);
  ...
  if (a) {
    uip_ipaddr_copy((uip_ip6addr_t *) &f.associationDestinationIpv6Address1,
                    &a->resource_ip);
                                         ...
                                         ip_association_t **result)
{
  ...
  *result                   = NULL;
  ...
  for (ip_association_t *a = list_head(ip_association_table); a != NULL;
       a                   = list_item_next(a)) {
    if ((a->han_nodeid == han_nodeid) && (a->han_endpoint == han_endpoint)
        && (a->grouping == grouping_id)) {
      nodes_in_grouping++;
      if (--index == 0) {
        *result = a;
      }
...
{
  (void) len;
  ZW_COMMAND_IP_ASSOCIATION_REMOVE *cmd =
    (ZW_COMMAND_IP_ASSOCIATION_REMOVE *) payload;
  uip_ip6addr_t zero_ip, ip;
  ...
  if (ip_assoc_is_any_marked_for_removal()) { /*Just locking further IP association removal till existing ones are finished */
    ERR_PRINTF("Still in the process of removing IP associations in response "
               "to previous IP_ASSOCIATION_REMOVE command.\n");
    send_zip_ack(TRANSMIT_COMPLETE_ERROR);
    report_send_completed(TRANSMIT_COMPLETE_ERROR);
    return;
  }
  memcpy(&ip, &cmd->ip_addr, sizeof(ip));
  ip_assoc_find_and_mark_for_removal(sl_node_of_ip(&c->lipaddr),
                                     c->lendpoint,
                                     cmd->grouping,
                                     &ip,
                                     cmd->endpoint);
  ...
  send_zip_ack(TRANSMIT_COMPLETE_OK);
  report_send_completed(
    TRANSMIT_COMPLETE_OK);   /* _OK because we dont want to see this frame again */
  return;
}
 ...
 *             Use sli_ip_assoc_create_local() to create a local IP association.
                    ...
                    uint8_t was_dtls)
{
  ...
  DBG_PRINTF("sli_ip_assoc_create of %s type\n", ip_assoc_type2str(type));
  ...
  if (a) {
    a->han_endpoint = sl_uip_buf_rendpoint();
    ...
    retVal                                       = ZW_SetSlaveLearnMode(0,
                                                                        VIRTUAL_SLAVE_LEARN_MODE_ADD,
                                                                        ip_assoc_virtual_add_callback);
    if (!retVal) {
      ip_assoc_virtual_add_callback_state.cmd = NULL;
    ...
    /* LOCAL_IP_ASSOC is unsupported here. Call sli_ip_assoc_create_local() instead */
...
{
  ip_association_t *new_assoc =
    (ip_association_t *) memb_alloc(&ip_association_pool);
  if (!new_assoc) {
  ...
  new_assoc->resource_port     = UIP_HTONS(ZWAVE_PORT); //UIP_HTONS(DTLS_PORT);
  new_assoc->resource_endpoint = cmd->endpoint;
  new_assoc->grouping          = cmd->groupingIdentifier;
  new_assoc->han_nodeid        = sl_node_of_ip(&sl_uip_buf_dst_addr());
  new_assoc->han_endpoint      = sl_uip_buf_dst_addr().u8[10];
                                          ...
                                          nodeid_t newID)
{
  ...
  ZW_COMMAND_IP_ASSOCIATION_SET *set_cmd =
    ip_assoc_virtual_add_callback_state.cmd;
  ip_assoc_virtual_add_callback_state.cmd = NULL;
  ...
  switch (bStatus) {
    case ASSIGN_NODEID_DONE:
      ...
      DBG_PRINTF("Successfully preallocated a virtual node %d for permanent "
                 "association\n",
                 newID);
...
static void ip_assoc_create_permanent_callback(ip_association_t *a)
{
  ...
  if (a->virtual_endpoint != 0) {
    ...
    *(p++) = a->virtual_endpoint;
    ...
    ZW_ASSOCIATION_SET_1BYTE_V2_FRAME *assoc_frame =
      (void *) (ClassicZIPNode_getTXBuf() + i);
...
static void ip_assoc_call_create_complete_cb(ip_association_t *a)
{
  ...
  zw_zip_post_event(ZIP_EVENT_COMPONENT_DONE, 0);
}
...
static void ip_assoc_add_to_association_table(ip_association_t *new_a)
{
  ...
  for (ip_association_t *a = list_head(ip_association_table); a != NULL;
    ...
    if (0 == memcmp(&new_a->virtual_id,
                    &a->virtual_id,
                    sizeof(ip_association_t)
                    - offsetof(ip_association_t, virtual_id))) {
...
{
  ip_association_t *a       = NULL;
  nodeid_t destination_node = sl_node_of_ip(&(sl_uip_buf_src_addr()));

  DBG_PRINTF("ip_assoc_find_existing\n");

  for (a = list_head(ip_association_table); a != NULL; a = list_item_next(a)) {
    if (a->type == type && destination_node == a->han_nodeid
        && uip_ipaddr_cmp(&a->resource_ip, &(sl_uip_buf_src_addr()))
  ...
  const ZW_COMMAND_IP_ASSOCIATION_SET *ip_assoc_set_cmd)
{
  ...
  new_assoc->han_endpoint = sl_uip_buf_rendpoint();

  /* The following check is really not needed (this function is only called for
   * PERMANENT_IP_ASSOC and PROXY_IP_ASSOC)
   */
  if (new_assoc->type == PERMANENT_IP_ASSOC
      || new_assoc->type == PROXY_IP_ASSOC) {
    if (sl_backup_zip_len() < IP_ASSOC_SET_FIXED_SIZE) {
    ...
    uip_ipaddr_copy(&(new_assoc->resource_ip),
                    (uip_ipaddr_t *) &(ip_assoc_set_cmd->resourceIP));
...
static ip_association_t *ip_assoc_find_first_marked_for_removal(void)
{
  for (ip_association_t *a = list_head(ip_association_table); a != NULL;
       a                   = list_item_next(a)) {
    if (a->mark_removal == 1) {
      DBG_PRINTF("ip_assoc_find_first_marked_for_removal found "
                 "a->resource_endpoint:%d\n ip:",
                 a->resource_endpoint);
...
static void ip_assoc_remove(ip_association_t *a)
{
  ...
  if (a->type == PERMANENT_IP_ASSOC) {
    ...
    BYTE retVal =
      ZW_SetSlaveLearnMode(a->virtual_id,
 ...
 */
static void ip_assoc_find_and_mark_for_removal(
  nodeid_t han_nodeid,
  uint8_t han_endpoint,
  uint8_t grouping_id,
  const uip_ip6addr_t *resource_ip,
  uint8_t resource_endpoint)   /* NB: resource_endpoint not used */
{
  ...
  for (ip_association_t *a = list_head(ip_association_table); a != NULL;
       a                   = list_item_next(a)) {
    if (a->han_nodeid == han_nodeid
        && ((han_endpoint == 0) || (a->han_endpoint == han_endpoint))
        && ((grouping_id == 0) || (a->grouping == grouping_id))
        && ((resource_ip == 0) || uip_ipaddr_cmp(resource_ip, &zero_ip)
            || uip_ipaddr_cmp(resource_ip, &a->resource_ip))) {
      a->mark_removal = 1;
      number_of_ip_assocs_marked_for_removal++;
      DBG_PRINTF("IP association marked for removal. HAN node id: %d, resource "
                 "endpoint: %d, resource ip:",
                 a->han_nodeid,
                 a->resource_endpoint);
      uip_debug_ipaddr_print(&a->resource_ip);
    }
...
ip_assoc_remove_callback(uint8_t bStatus, void *user, TX_STATUS_TYPE *t)
{
  ...
  if ((bStatus == TRANSMIT_COMPLETE_OK)
      || (bStatus == TRANSMIT_COMPLETE_NO_ACK)) {
    ip_assoc_remove_and_persist(a);
  } else {
    send_zip_ack(
      TRANSMIT_COMPLETE_FAIL);   /* Indicate delivery failure to source */
    report_send_completed(
      TRANSMIT_COMPLETE_OK);   /* _OK because we dont want to see this frame again */
  }
}
                                             ...
                                             TX_STATUS_TYPE *t)
{
  ...
  if ((a != NULL) && (bStatus == TRANSMIT_COMPLETE_OK)) {
    ...
    if (!ZW_AssignReturnRoute(a->han_nodeid,
                              a->type == LOCAL_IP_ASSOC
                              ? sl_node_of_ip(&a->resource_ip)
                              : a->virtual_id,
  ...
  ZW_COMMAND_IP_ASSOCIATION_SET *ip_assoc_cmd)
{
  ...
  uip_ip6addr_t ip;
  memcpy(&ip, &ip_assoc_cmd->resourceIP, sizeof(ip));
  b->nodeId1 = sl_node_of_ip(&ip);
  return 4;
}
                                                       ...
                                                       ip_association_t *ipa)
{
  ...
  if (ipa->type == LOCAL_IP_ASSOC) {
    b->nodeId1 = sl_node_of_ip(&ipa->resource_ip);
  } else {
                                              ...
                                              ip_association_t *ipa)
{
  ...
  *(p++) = ipa->type == LOCAL_IP_ASSOC ? sl_node_of_ip(&ipa->resource_ip)
           : ipa->virtual_id;
  *(p++) = ipa->type == LOCAL_IP_ASSOC ? ipa->resource_endpoint
           : ipa->virtual_endpoint;
  return 6;
}
...
static void ip_assoc_setup_and_send_assoc_remove_cmd(ip_association_t *ipa)
{
  ...
  if (sl_cmdclass_supported(ipa->han_nodeid,
                            COMMAND_CLASS_MULTI_CHANNEL_ASSOCIATION_V2)
      || sl_cmdclass_secure_supported(ipa->han_nodeid,
                                      COMMAND_CLASS_MULTI_CHANNEL_ASSOCIATION_V2)) {
    /* Case 5 */
    i = ip_assoc_setup_multi_channel_assoc_remove_cmd(ClassicZIPNode_getTXBuf(),
                                                      ipa);
  } else {
    /* Case 2 */
  ...
  if (!ClassicZIPNode_SendDataAppl(
        &p,
...
ip_association_t *ip_assoc_head()
{
  return (ip_association_t *) list_head(ip_association_table);
}

ip_association_t *ip_assoc_next(ip_association_t *a)
{
