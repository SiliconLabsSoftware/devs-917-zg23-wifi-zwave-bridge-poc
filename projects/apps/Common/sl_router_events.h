...
enum {
  ...
  /**
   * Emitted when a node has got a new ip address, the data argument contains the nodeid
   */
  ZIP_EVENT_NODE_IPV4_ASSIGNED,

  /**
   * Emitted when the DHCP process has given up trying to acquire an IP address. The data
  ...
  /**
   * The DHCP discover/renew round has completed, we have acquired an
   ...
   * probe (ending in STATUS_PROBE_FAIL or STATUS_DONE).
   */
