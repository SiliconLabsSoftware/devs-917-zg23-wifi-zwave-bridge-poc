...
#include "apps/transport/sl_ts_param.h"
 ...
 * communication with a node was successful.
 *
...
/** Called from \ref sl_appl_controller_update, when a node info is
...
BYTE
sl_set_cache_entry_flag_masked(nodeid_t nodeid, BYTE value, BYTE mask);
BYTE
sl_get_cache_entry_flag(nodeid_t nodeid);

int
isNodeController(nodeid_t nodeid);

int
sl_cmdclass_supported(nodeid_t nodeid, WORD class);
