...
static void secure_learn_clearOutEvents(Secure_learn* handle)
{
  (void)handle; // Mark as unused to avoid -Wunused-parameter
  // Reserved for future use, currently no out events to clear
}
...
{
  // Cast both sides to intptr_t for correct signed/unsigned comparison
  if ( ((intptr_t)evid) >= ((intptr_t)&(handle->timeEvents))
       &&  ((intptr_t)evid) < (intptr_t)(((uint8_t*)&(handle->timeEvents)) + sizeof(Secure_learnTimeEvents))) {
...
sc_boolean secure_learn_isActive(Secure_learn* handle, Secure_learnStates state)
{
  switch (state) {
    case Secure_learn_main_region_LearnMode:
      // Fix always-true comparison: cast both sides to sc_integer for correct signed comparison
