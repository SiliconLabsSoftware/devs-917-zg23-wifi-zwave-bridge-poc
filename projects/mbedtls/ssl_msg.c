                            ...
                            void *p_rng)
{
    ...
    if (rec->data_len > MBEDTLS_SSL_OUT_CONTENT_LEN) {
        ...
        printf("record content: %d, too large, max: %d\n", rec->data_len,
                                  (size_t) MBEDTLS_SSL_OUT_CONTENT_LEN);
...
int mbedtls_ssl_fetch_input(mbedtls_ssl_context *ssl, size_t nb_want)
{
    ...
    if (ssl->f_recv == NULL && ssl->f_recv_timeout == NULL) {
        ...
        printf("Bad usage of mbedtls_ssl_set_bio(): %p, %p\n", ssl->f_recv, ssl->f_recv_timeout);
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }

    if (nb_want > in_buf_len - (size_t) (ssl->in_hdr - ssl->in_buf)) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("requesting more data than fits"));
        printf("requesting more data than fits, %d, %d - %d\n", nb_want, in_buf_len, (ssl->in_hdr - ssl->in_buf));
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
                            ...
                            unsigned update_hs_digest)
{
    ...
    if (ssl->keep_current_message == 0) {
        do {
            ...
            printf("ssl_record_is_in_progress\n");
            ...
            printf("mbedtls_ssl_handle_message_type\n");
        ...
        if (ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE &&
            update_hs_digest == 1) {
            printf("mbedtls_ssl_update_handshake_status\n");
